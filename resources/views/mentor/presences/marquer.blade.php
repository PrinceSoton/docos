...
@section('content')
    <div class="space-y-4 sm:space-y-5">
        ...
        <div data-aos="fade-down">
            ...
            <p class="text-sm text-slate-500">Marquez ou corrigez les présences de vos stagiaires</p>
        </div>

        <div class="card p-5" data-aos="fade-up">
            <form method="GET" action="{{ route('mentor.presences.formMarquage') }}"
                ...
                <button type="submit"
                    class="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-2.5 rounded-xl font-medium">
        ...
        @else
            <form action="{{ route('mentor.presences.marquerLot') }}" method="POST" class="no-loader">
                @csrf
                <input type="hidden" name="date" value="{{ $date->toDateString() }}">

                <div class="card overflow-hidden" data-aos="fade-up">
                    <div
                        class="px-5 py-4 border-b border-slate-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                        ...
                        <table class="w-full text-sm">
                            <thead class="bg-slate-50">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-left font-semibold text-slate-600 hidden sm:table-cell">Motif
                                    </th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-slate-50">
                                @foreach ($stagiaires as $i => $stag)
                                    ...
                                    <tr class="hover:bg-slate-50">
                                        <td class="px-4 py-3">
                                            <input type="hidden" name="presences[{{ $i }}][stagiaire_id]"
                                                value="{{ $stag->id }}">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p class="font-semibold text-slate-800">{{ $stag->user->nom_complet }}
                                                    </p>
                                            ...
                                            <div class="flex justify-center gap-1 sm:gap-2 flex-wrap">
                                                @foreach (['present' => 'Présent', 'retard' => 'Retard', 'absent' => 'Absent'] as $val => $label)
                                                    <label class="cursor-pointer">
                                                        <input type="radio" name="presences[{{ $i }}][statut]"
                                                            value="{{ $val }}" class="peer sr-only"
                                                            {{ $p ? ($p->statut === $val ? 'checked' : '') : ($val === 'present' ? 'checked' : '') }}>
                                                        <span
                                                            class="block px-2 sm:px-3 py-1.5 rounded-lg border-2 border-slate-200 text-xs font-medium
                                                                     peer-checked:border-emerald-500 peer-checked:bg-emerald-50 peer-checked:text-emerald-700 transition">
                                                            {{ $label }}
                                                        </span>
                                                    </label>
                                                @endforeach
                                        ...
                                        <td class="px-4 py-3 hidden sm:table-cell">
                                            <input type="text" name="presences[{{ $i }}][motif]"
                                                value="{{ $p->motif ?? '' }}" placeholder="Motif si absent / retard..."
                                                class="w-full sm:w-56 border border-slate-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-emerald-400">
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                ...
                <div class="flex flex-col sm:flex-row gap-3 mt-4">
                    <button type="submit"
                        class="flex-1 sm:flex-none bg-gradient-to-r from-emerald-600 to-teal-600 text-white px-8 py-3 rounded-xl font-semibold hover:shadow-lg hover:-translate-y-0.5 transition-all">
