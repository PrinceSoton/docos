    ...
    <div class="space-y-4 sm:space-y-5">
        ...
        <div class="card p-5" data-aos="fade-up">
            <form action="{{ route('mentor.presences.index') }}" method="GET"
                class="flex flex-col sm:flex-row gap-3 items-end no-loader">
                ...
                <button type="submit"
                    class="w-full sm:w-auto bg-emerald-600 text-white px-6 py-2.5 rounded-xl font-medium hover:bg-emerald-700 transition hover:shadow-md">
                    <i class="fas fa-search mr-2"></i>Filtrer
                </button>
                @if (request('stagiaire_id'))
                    <a href="{{ route('mentor.presences.show', request('stagiaire_id')) }}"
                        class="w-full sm:w-auto bg-indigo-600 text-white px-6 py-2.5 rounded-xl font-medium hover:bg-indigo-700 transition hover:shadow-md">
            ...
            <div class="card overflow-hidden" data-aos="fade-up">
                <div
                    ...
                    <input type="text" id="search" placeholder="Rechercher..."
                        class="w-full sm:w-auto border border-slate-200 rounded-xl px-4 py-2 text-sm focus:outline-none focus:border-emerald-400 transition">
                </div>
                    ...
                    <table class="w-full text-sm">
                        <thead class="bg-slate-50">
                            <tr>
                                ...
                                @if (!$stagiaire)
                                    <th class="px-4 sm:px-5 py-3 text-left font-semibold text-slate-600">Stagiaire</th>
                                @endif
                        ...
                        <tbody class="divide-y divide-slate-50" id="presTable">
                            @foreach ($presences as $p)
                                <tr class="hover:bg-slate-50 transition pres-row">
                                    ...
                                    @if (!$stagiaire)
                                        <td class="px-4 sm:px-5 py-3 text-slate-600">
                                            {{ $p->stagiaire->user->nom_complet ?? '—' }}</td>
                                    @endif
