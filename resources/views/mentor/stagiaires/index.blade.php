...
@section('content')
    <div class="space-y-4 sm:space-y-5">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3" data-aos="fade-down">
            <div>
                <h2 class="text-xl sm:text-2xl font-bold text-slate-800">Mes Stagiaires</h2>
                <p class="text-sm text-slate-500">{{ $stagiaires->total() }} stagiaire(s) sous votre encadrement</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-5" id="stagGrid">
            @forelse($stagiaires as $stag)
                <div class="card overflow-hidden stag-card hover:shadow-xl transition-all duration-300 hover:-translate-y-1"
                    ...
                    <div class="pt-9 px-4 sm:px-5 pb-5">
                        ...
                        <div class="mt-3 space-y-1.5 text-sm">
                            <div class="flex items-center gap-2 text-slate-600"><i
                                    class="fas fa-graduation-cap text-emerald-400 w-4"></i><span>{{ $stag->ecole ?: '—' }}</span>
                            </div>
                            <div class="flex items-center gap-2 text-slate-600"><i
                                    class="fas fa-code-branch text-emerald-400 w-4"></i><span>{{ $stag->specialite ?: '—' }}</span>
                            </div>
                            <div class="flex items-center gap-2 text-slate-600"><i
                                    class="fas fa-calendar-alt text-emerald-400 w-4"></i><span>{{ $stag->date_debut->format('d/m/Y') }}
                                    → {{ $stag->date_fin->format('d/m/Y') }}</span></div>
                        </div>
                        @php
                        ...
                        <div class="mt-4">
                            ...
                            <div class="h-2 bg-slate-100 rounded-full overflow-hidden">
                                <div class="h-2 rounded-full transition-all duration-700 {{ $pct >= 75 ? 'bg-gradient-to-r from-emerald-400 to-green-500' : ($pct >= 40 ? 'bg-gradient-to-r from-amber-400 to-orange-400' : 'bg-gradient-to-r from-red-400 to-pink-400') }}"
                                    style="width:{{ $pct }}%"></div>
                            </div>
                        </div>
            ...
            @empty
                <div class="col-span-1 sm:col-span-2 xl:col-span-3 card p-8 sm:p-16 text-center">
                    <i class="fas fa-user-graduate text-4xl sm:text-5xl text-slate-200 mb-4"></i>
