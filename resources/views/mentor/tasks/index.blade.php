...
@section('content')
    <div class="space-y-4 sm:space-y-5">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3" data-aos="fade-down">
            <div>
                <h2 class="text-xl sm:text-2xl font-bold text-slate-800">Tâches</h2>
                <p class="text-sm text-slate-500">Vue Kanban de toutes les tâches de vos projets</p>
            </div>
        ...
        @forelse($projets as $projet)
                ...
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-5 gap-3">
                    <div>
                        <h3 class="text-slate-800 font-bold text-lg"><i
                                class="fas fa-folder text-emerald-500 mr-2"></i>{{ $projet->titre }}</h3>
                    ...
                    @foreach ([['a_faire', 'À Faire', 'slate', 'clock'], ['en_cours', 'En Cours', 'amber', 'spinner fa-spin'], ['termine', 'Terminé', 'green', 'check-circle']] as [$statut, $label, $color, $icon])
                        ...
                        <div class="bg-{{ $color }}-50 rounded-xl p-4 border border-{{ $color }}-100">
                            <div class="flex items-center justify-between mb-3">
                                ...
                                <span
                                    class="w-5 h-5 rounded-full bg-{{ $color }}-200 text-{{ $color }}-700 text-xs font-bold flex items-center justify-center">{{ $taches->count() }}</span>
                            </div>
                            <div class="space-y-2 tache-col" data-statut="{{ $statut }}">
                                @forelse($taches as $tache)
                                    <div
                                        ...
                                        <p class="text-xs text-slate-400 mb-2"><i
            ...
            @empty
                <div class="card p-8 sm:p-16 text-center" data-aos="fade-up">
                    <i class="fas fa-tasks text-4xl sm:text-5xl text-slate-200 mb-4"></i>
