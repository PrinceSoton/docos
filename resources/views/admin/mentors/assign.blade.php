...
@section('content')
    <div class="max-w-2xl mx-auto px-4 sm:px-0 space-y-6">
            ...
            <form action="{{ route('admin.mentors.doAssign') }}" method="POST" class="space-y-5">
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2"><i
                            class="fas fa-user-graduate mr-1 text-indigo-500"></i>Stagiaire *</label>
                    <select name="stagiaire_id" required
                        ...
                        @foreach ($stagiaires as $stag)
                            <option value="{{ $stag->id }}" {{ old('stagiaire_id') == $stag->id ? 'selected' : '' }}>
                                {{ $stag->user->nom_complet }} — {{ $stag->matricule }} @if ($stag->mentor_id)
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2"><i
                            class="fas fa-chalkboard-teacher mr-1 text-emerald-500"></i>Mentor *</label>
                    <select name="mentor_id" required
                        ...
                        @foreach ($mentors as $mentor)
                            <option value="{{ $mentor->user_id }}"
                                ...
                                {{ $mentor->user->nom_complet }} @if ($mentor->departement)
                                    — {{ $mentor->departement }}
                                @endif ({{ $mentor->stagiaires->count() }} stagiaire(s))</option>
                        @endforeach
                    </select>
                </div>
        ...
        <div class="card p-5 sm:p-6" data-aos="fade-up" data-aos-delay="100">
            <h3 class="text-slate-800 font-bold text-lg mb-4"><i
                    class="fas fa-sitemap text-indigo-500 mr-2"></i>Affectations actuelles</h3>
            <div class="space-y-3">
                @foreach ($mentors as $mentor)
