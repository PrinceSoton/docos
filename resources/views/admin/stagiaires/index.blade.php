...
@section('content')
    <div class="space-y-4 sm:space-y-5">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3" data-aos="fade-down">
            <div>
                <h2 class="text-xl sm:text-2xl font-bold text-slate-800">Stagiaires</h2>
                <p class="text-sm text-slate-500">{{ $stagiaires->total() }} stagiaire(s) enregistré(s)</p>
            </div>
        ...
        <div class="card p-4 flex flex-col sm:flex-row gap-3 items-center" data-aos="fade-up">
            <input type="text" id="search" placeholder="Nom, matricule, école, spécialité..."
                class="flex-1 w-full border border-slate-200 rounded-xl px-4 py-2.5 focus:outline-none focus:border-indigo-400 text-sm transition">
            <select id="filterStatut"
                class="w-full sm:w-auto border border-slate-200 rounded-xl px-4 py-2.5 focus:outline-none focus:border-indigo-400 text-sm transition">
        ...
        <div class="card overflow-hidden" data-aos="fade-up">
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gradient-to-r from-slate-50 to-slate-100">
                        <tr>
                            ...
                            <th class="px-4 sm:px-5 py-4 text-left font-semibold text-slate-600 hidden sm:table-cell">
                                Matricule</th>
                            <th class="px-4 sm:px-5 py-4 text-left font-semibold text-slate-600 hidden md:table-cell">École
                                / Spécialité</th>
                            <th class="px-4 sm:px-5 py-4 text-left font-semibold text-slate-600 hidden lg:table-cell">Mentor
                            </th>
                            <th class="px-4 sm:px-5 py-4 text-left font-semibold text-slate-600 hidden xl:table-cell">
                                Période</th>
                            <th class="px-4 sm:px-5 py-4 text-center font-semibold text-slate-600">Statut</th>
                    ...
                    <tbody class="divide-y divide-slate-50" id="stagTable">
                        @forelse($stagiaires as $stag)
                            <tr class="hover:bg-slate-50 transition stag-row" data-statut="{{ $stag->statut }}">
                                ...
                                <td class="px-4 sm:px-5 py-4 text-slate-600 text-sm hidden lg:table-cell">
                                    {{ $stag->mentor?->nom_complet ?: '—' }}</td>
                                <td class="px-4 sm:px-5 py-4 hidden xl:table-cell">
                                ...
                                <td class="px-4 sm:px-5 py-4 text-center">
                                    <span
                                        class="px-3 py-1 rounded-full text-xs font-semibold {{ $stag->statut === 'en_cours' ? 'bg-green-100 text-green-700' : ($stag->statut === 'termine' ? 'bg-slate-100 text-slate-600' : 'bg-red-100 text-red-600') }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="text-center py-16"><i
                                        class="fas fa-user-graduate text-5xl text-slate-200 mb-3"></i>
            ...
            <div class="px-4 sm:px-5 py-4 border-t border-slate-100">{{ $stagiaires->links() }}</div>
        </div>
    </div>
@endsection
@push('scripts')
