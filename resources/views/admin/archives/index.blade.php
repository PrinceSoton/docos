...
@section('content')
    <div class="space-y-4 sm:space-y-5">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3" data-aos="fade-down">
            <div>
                <h2 class="text-xl sm:text-2xl font-bold text-slate-800">Archives</h2>
                <p class="text-sm text-slate-500">{{ $archives->total() }} archive(s)</p>
            </div>
        ...
        <div class="card overflow-hidden" data-aos="fade-up">
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="bg-gradient-to-r from-slate-50 to-slate-100">
                        <tr>
                            <th class="px-4 sm:px-5 py-4 text-left font-semibold text-slate-600">Titre</th>
                            ...
                            <th class="px-4 sm:px-5 py-4 text-left font-semibold text-slate-600 hidden md:table-cell">
                                Stagiaire</th>
                            <th class="px-4 sm:px-5 py-4 text-center font-semibold text-slate-600">Fichiers</th>
                            <th class="px-4 sm:px-5 py-4 text-left font-semibold text-slate-600 hidden lg:table-cell">Créé
                                par</th>
                            <th class="px-4 sm:px-5 py-4 text-left font-semibold text-slate-600 hidden xl:table-cell">Date
                            </th>
                        ...
                        @forelse($archives as $archive)
                            <tr class="hover:bg-slate-50 transition archive-row" data-type="{{ $archive->type }}">
                                ...
                                <td class="px-4 sm:px-5 py-4 hidden sm:table-cell">
                                    <span
                                        class="px-3 py-1 rounded-full text-xs font-semibold {{ $archive->type === 'stagiaire' ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-600' }}">
                                        <i
                                            class="fas fa-{{ $archive->type === 'stagiaire' ? 'user-graduate' : 'folder' }} mr-1"></i>{{ ucfirst($archive->type) }}
                                    </span>
                                </td>
                                <td class="px-4 sm:px-5 py-4 text-slate-600 text-sm hidden md:table-cell">
                                ...
                                <td class="px-4 sm:px-5 py-4 text-center">
                                    <span
                                        class="px-3 py-1 rounded-full text-xs font-bold bg-amber-100 text-amber-700">{{ $archive->fichiers->count() }}</span>
                                </td>
                                <td class="px-4 sm:px-5 py-4 text-slate-600 text-sm hidden lg:table-cell">
                                    {{ $archive->creePar?->nom_complet ?? '—' }}</td>
                                <td class="px-4 sm:px-5 py-4 text-slate-400 text-xs hidden xl:table-cell">
                                    {{ $archive->created_at->format('d/m/Y') }}</td>
                                <td class="px-4 sm:px-5 py-4">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="text-center py-16"><i
                                        class="fas fa-archive text-5xl text-slate-200 mb-3"></i>
            ...
            <div class="px-4 sm:px-5 py-4 border-t border-slate-100">{{ $archives->links() }}</div>
        </div>
    </div>
@endsection
@push('scripts')
