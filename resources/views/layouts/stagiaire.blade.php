    ...
    <div class="loader-overlay" id="appLoader">
        <div class="relative mb-4 w-20 h-20 mx-auto">
            ...
            <img src="{{ asset('logo.png') }}" alt="DOCOS"
                class="w-14 h-14 object-contain absolute inset-0 m-auto rounded-xl">
        </div>
        ...
        <aside id="sidebar" class="sidebar flex-shrink-0 flex flex-col shadow-2xl z-50">
            ...
            <div class="px-5 py-4 border-b border-white/10">
                <div class="flex items-center gap-3">
                    <div
                        class="w-9 h-9 rounded-full bg-gradient-to-br from-amber-400 to-orange-400 flex items-center justify-center flex-shrink-0">
                        @if (Auth::user()->photo)
            ...
            <nav class="flex-1 py-4 overflow-y-auto">
                <a href="{{ route('stagiaire.dashboard') }}"
                    class="sidebar-item flex items-center gap-3 px-4 py-3 text-white/80 hover:text-white {{ request()->routeIs('stagiaire.dashboard') ? 'active' : '' }}">
                    <i class="fas fa-home w-5 text-center flex-shrink-0"></i><span
                        class="sidebar-label text-sm font-medium">Tableau de bord</span>
                </a>
                <a href="{{ route('stagiaire.presence.index') }}"
                    class="sidebar-item flex items-center gap-3 px-4 py-3 text-white/80 hover:text-white {{ request()->routeIs('stagiaire.presence.*') ? 'active' : '' }}">
                    <i class="fas fa-user-check w-5 text-center flex-shrink-0"></i><span
                        class="sidebar-label text-sm font-medium">Ma Présence</span>
                </a>
                <a href="{{ route('stagiaire.projects.index') }}"
                    class="sidebar-item flex items-center gap-3 px-4 py-3 text-white/80 hover:text-white {{ request()->routeIs('stagiaire.projects.*') ? 'active' : '' }}">
                    <i class="fas fa-project-diagram w-5 text-center flex-shrink-0"></i><span
                        class="sidebar-label text-sm font-medium">Mon Projet</span>
                </a>
                <a href="{{ route('stagiaire.tasks.index') }}"
                    class="sidebar-item flex items-center gap-3 px-4 py-3 text-white/80 hover:text-white {{ request()->routeIs('stagiaire.tasks.*') ? 'active' : '' }}">
                    <i class="fas fa-check-square w-5 text-center flex-shrink-0"></i><span
                        class="sidebar-label text-sm font-medium">Mes Tâches</span>
                </a>
                <a href="{{ route('stagiaire.reports.index') }}"
                    class="sidebar-item flex items-center gap-3 px-4 py-3 text-white/80 hover:text-white {{ request()->routeIs('stagiaire.reports.*') ? 'active' : '' }}">
                    <i class="fas fa-file-upload w-5 text-center flex-shrink-0"></i><span
                        class="sidebar-label text-sm font-medium">Mes Rapports</span>
                </a>
                <a href="{{ route('stagiaire.attestations.index') }}"
                    class="sidebar-item flex items-center gap-3 px-4 py-3 text-white/80 hover:text-white {{ request()->routeIs('stagiaire.attestations.*') ? 'active' : '' }}">
                ...
                <a href="{{ route('stagiaire.evenements.index') }}"
                    class="sidebar-item flex items-center gap-3 px-4 py-3 text-white/80 hover:text-white {{ request()->routeIs('stagiaire.evenements.*') ? 'active' : '' }}">
            ...
            <div class="p-4 border-t border-white/10">
                <a href="{{ route('profile.index') }}"
                    ...
                    <i class="fas fa-user-cog w-5 text-center flex-shrink-0"></i><span
                        class="sidebar-label text-sm font-medium">Mon Profil</span>
                </a>
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit"
                        class="sidebar-item flex items-center gap-3 px-4 py-3 text-red-400 hover:text-red-300 w-full text-left">
                        <i class="fas fa-sign-out-alt w-5 text-center flex-shrink-0"></i><span
                            class="sidebar-label text-sm font-medium">Déconnexion</span>
                    </button>
                </form>
            </div>
        </aside>
