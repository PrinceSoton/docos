...
@section('content')
    <div class="space-y-4 sm:space-y-5">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3" data-aos="fade-down">
            <div>
                <h2 class="text-xl sm:text-2xl font-bold text-slate-800">Mes Rapports</h2>
                <p class="text-sm text-slate-500">{{ $rapports->total() }} rapport(s) déposé(s)</p>
            </div>
            <a href="{{ route('stagiaire.reports.create') }}"
                class="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white px-5 py-2.5 rounded-xl font-medium hover:shadow-lg hover:-translate-y-0.5 transition-all">
        ...
        <div class="card p-4 flex flex-col sm:flex-row gap-3" data-aos="fade-up">
            ...
            <select id="filterStatut"
                class="w-full sm:w-auto border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-indigo-400">
        ...
        <!-- Grille -->
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5" id="rapportGrid">
            @forelse($rapports as $rapport)
                <div class="card overflow-hidden rapport-card hover:shadow-xl transition-all duration-300 hover:-translate-y-1"
                    ...
                    @php $sc = ['soumis' => 'bg-amber-400', 'valide' => 'bg-green-400', 'rejete' => 'bg-red-400', 'en_revision' => 'bg-blue-400']; @endphp
                    <div class="h-1.5 {{ $sc[$rapport->statut] ?? 'bg-slate-300' }}"></div>
                    <div class="p-4 sm:p-5">
                        <div class="flex items-start justify-between gap-2 mb-3">
                            <div>
                                <h3 class="font-bold text-slate-800 text-base sm:text-lg">{{ $rapport->titre }}</h3>
                            </div>
                        ...
                        @endif
                        @if ($rapport->project)
                            <p class="text-xs text-indigo-600 font-medium mb-2"><i
                                    class="fas fa-project-diagram mr-1"></i>{{ $rapport->project->titre }}</p>
                        @endif
            ...
            @empty
                <div class="col-span-1 sm:col-span-2 lg:col-span-3 card p-8 sm:p-16 text-center">
                    <i class="fas fa-file-alt text-4xl sm:text-5xl text-slate-200 mb-4"></i>
