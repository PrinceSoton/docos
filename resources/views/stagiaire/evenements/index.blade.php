...
@section('content')
    <div class="space-y-5">
        <div data-aos="fade-down">
            ...
            <p class="text-slate-500 text-sm">Communications de l'entreprise</p>
        </div>

        <div class="card p-4 flex flex-wrap gap-3" data-aos="fade-up">
            <input type="text" id="search" placeholder="Rechercher..."
                class="flex-1 min-w-48 border border-slate-200 rounded-xl px-4 py-2.5 focus:outline-none focus:border-amber-400 text-sm">
            <select id="filterType"
                class="border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-amber-400">
    ...
    <script>
        function f() {
            ...
            document.querySelectorAll('.evt-card').forEach(c => {
                c.style.display = c.textContent.toLowerCase().includes(s) && (!t || c.dataset.type === t) ? '' :
                    'none';
            });
        }
        document.getElementById('search').addEventListener('input', f);
        document.getElementById('filterType').addEventListener('change', f);
    </script>
@endpush
