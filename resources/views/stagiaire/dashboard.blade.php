...
@section('content')
    <div class="space-y-6">
        ...
        <div class="bg-gradient-to-r from-slate-800 via-indigo-900 to-slate-900 rounded-2xl sm:rounded-3xl p-6 sm:p-8 text-white relative overflow-hidden"
            data-aos="fade-down">
            <div class="absolute right-0 top-0 w-64 h-64 bg-amber-500/10 rounded-full -translate-y-1/3 translate-x-1/3">
            </div>
            ...
            <div class="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                ...
                <div class="text-right flex-shrink-0">
                    ...
                    <p class="text-3xl sm:text-4xl font-black text-amber-400">{{ $joursRestants }}</p>
        ...
        <!-- Résumé des tâches (Kanban simplifié) -->
        <div class="card p-5 sm:p-6" data-aos="fade-up">
            ...
            <div class="grid grid-cols-3 gap-3 sm:gap-4">
                @foreach ([['a_faire', $stats['a_faire'], 'À faire', 'slate', 'clock'], ['en_cours', $stats['en_cours'], 'En cours', 'amber', 'spinner'], ['terminees', $stats['terminees'], 'Terminées', 'green', 'check-circle']] as [$key, $val, $label, $color, $icon])
                    <div
                        class="bg-{{ $color }}-50 border border-{{ $color }}-100 rounded-2xl p-4 text-center">
                        <div
                        ...
                        <p class="text-2xl sm:text-3xl font-black text-{{ $color }}-600">{{ $val }}</p>
                        <p class="text-slate-500 text-xs sm:text-sm mt-1">{{ $label }}</p>
                    </div>
                @endforeach
            </div>
        ...
        <!-- Tâches récentes & Rapports récents -->
                ...
                @forelse($tachesRecentes as $tache)
                    <div class="flex items-center gap-3 py-2.5 border-b border-slate-50 last:border-0">
                        ...
                        <span
                            ...
                            {{ $tache->statut === 'a_faire' ? 'bg-slate-400' : ($tache->statut === 'en_cours' ? 'bg-amber-400' : 'bg-green-400') }}">
                        </span>
