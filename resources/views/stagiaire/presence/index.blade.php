...
@section('content')
    <div class="space-y-4 sm:space-y-6">
        ...
        <div class="bg-green-50 border border-green-200 rounded-xl p-3 sm:p-4" data-aos="fade-up">
            ...
            <div class="flex flex-wrap gap-x-3 gap-y-1 text-xs text-slate-600 mt-1">
                <span><span class="text-green-600">●</span> Présent avant
                    {{ \Carbon\Carbon::parse($heureDebut)->format('H:i') }}</span>
                <span><span class="text-amber-600">●</span> Retard entre
                    {{ \Carbon\Carbon::parse($heureDebut)->format('H:i') }} et
                    {{ \Carbon\Carbon::parse($heureFin)->format('H:i') }}</span>
                <span><span class="text-red-600">●</span> Absent après
                    {{ \Carbon\Carbon::parse($heureFin)->format('H:i') }}</span>
            </div>
        </div>
        ...
        <div class="card p-5 sm:p-6 border-l-4 {{ $peutMarquer ? 'border-green-400' : 'border-slate-300' }}"
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                <div
                    class="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl {{ $peutMarquer ? 'bg-green-100' : 'bg-slate-100' }} flex items-center justify-center flex-shrink-0">
                    <i
        ...
        @if ($peutMarquer)
            <div class="card p-5 sm:p-6" data-aos="fade-up" id="presenceForm" style="display:none">
                ...
                <form action="{{ route('stagiaire.presence.marquer') }}" method="POST" enctype="multipart/form-data"
                    ...
                    @csrf
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Motif de retard</label>
        ...
        <div class="card p-5 sm:p-6" data-aos="fade-up">
            ...
            <div id="permissionForm" style="display:none"
                class="p-4 sm:p-5 bg-purple-50 rounded-2xl border border-purple-100 mb-4">
            ...
            @if ($permissions->count() > 0)
                <div class="space-y-2">
                    @foreach ($permissions as $perm)
                        <div
                            class="flex flex-col sm:flex-row items-start sm:items-center gap-3 p-4 rounded-xl {{ $perm->statut === 'valide' ? 'bg-green-50 border border-green-100' : ($perm->statut === 'refuse' ? 'bg-red-50 border border-red-100' : 'bg-amber-50 border border-amber-100') }}">
                            <div class="flex-1">
                                <p class="text-slate-800 font-medium text-sm">
                                    {{ $perm->date_debut->format('d/m/Y') }} → {{ $perm->date_fin->format('d/m/Y') }}
                                    <span class="text-slate-400">({{ $perm->date_debut->diffInDays($perm->date_fin) + 1 }}
                                        jour(s))</span>
                                </p>
                                <p class="text-slate-500 text-xs mt-0.5">{{ $perm->motif }}</p>
                                @if ($perm->commentaire_mentor)
                                    <p class="text-slate-400 text-xs mt-0.5 italic"><i
                                            class="fas fa-comment mr-1"></i>{{ $perm->commentaire_mentor }}</p>
                                @endif
                            </div>
                            <div class="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
                                ...
                                <p class="text-slate-400 text-xs">{{ $perm->demande_le->format('d/m/Y') }}</p>
                                @if ($perm->justificatif)
                                    <a href="{{ route('stagiaire.presence.telechargerJustificatif', $perm->id) }}"
                                        download
        ...
        <div class="card overflow-hidden">
            ...
            <div class="grid grid-cols-3 gap-0 border-b border-slate-100">
                <div class="p-3 sm:p-4 text-center border-r border-slate-100">
                    <p class="text-xl sm:text-2xl font-black text-green-600">{{ $nbPresent }}</p>
                    <p class="text-xs text-slate-400">Présent</p>
                </div>
                <div class="p-3 sm:p-4 text-center border-r border-slate-100">
                    <p class="text-xl sm:text-2xl font-black text-amber-600">{{ $nbRetard }}</p>
                    <p class="text-xs text-slate-400">Retard</p>
                </div>
                <div class="p-3 sm:p-4 text-center">
                    <p class="text-xl sm:text-2xl font-black text-red-600">{{ $nbAbsent }}</p>
