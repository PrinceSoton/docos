...
@section('content')
    <div class="space-y-4 sm:space-y-5">
        <div data-aos="fade-down">
            <h2 class="text-xl sm:text-2xl font-bold text-slate-800">Mon Projet</h2>
            <p class="text-sm text-slate-500">{{ $projets->total() }} projet(s) assigné(s)</p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5" id="projetGrid">
            @forelse($projets as $projet)
                <div class="card overflow-hidden projet-card hover:shadow-xl transition-all duration-300 hover:-translate-y-1"
                    ...
                    @php $gradients = ['faible' => 'from-slate-400 to-slate-500', 'normale' => 'from-blue-400 to-indigo-500', 'haute' => 'from-amber-400 to-orange-500', 'urgente' => 'from-red-400 to-pink-500']; @endphp
                    <div class="h-2 bg-gradient-to-r {{ $gradients[$projet->priorite] ?? 'from-slate-300 to-slate-400' }}">
                    </div>
                    <div class="p-5 sm:p-6">
                        <div class="flex items-start justify-between gap-2 mb-3">
                            <h3 class="font-bold text-slate-800 text-lg sm:text-xl">{{ $projet->titre }}</h3>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="h-2.5 bg-slate-100 rounded-full overflow-hidden">
                                <div class="h-2.5 bg-gradient-to-r from-green-500 to-emerald-600 rounded-full"
                        ...
                        <a href="{{ route('stagiaire.projects.show', $projet) }}"
                            class="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white py-2.5 rounded-xl font-medium hover:shadow-lg hover:-translate-y-0.5 transition-all text-sm">
